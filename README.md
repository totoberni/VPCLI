...
### 2. Extract Assets
The required `.pkl` data artifacts are large and must be stored in a compressed format. Before running the application, extract the contents of the `assets.zip` file. This will create the necessary `assets/` directory at the root of the project.

You can use the built-in tools on your operating system or run one of the following commands in your terminal:
```bash
# Windows (PowerShell)
Expand-Archive -Path ./assets.zip -DestinationPath .
...
This guide is for developers who want to run and modify the project locally.
...
### 2. Extract Assets
The required `.pkl` data artifacts are large and must be stored in a compressed format. Before running the application, extract the contents of the `assets.zip` file into an `assets/` directory at the root of the project.
